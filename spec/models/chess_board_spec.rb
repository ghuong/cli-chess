...
describe ChessBoard do
  ...
  let(:id) { 12 }
  let(:dump_and_load) { ChessBoard.from_yaml(subject.to_yaml) }
  ...
  context 'when empty' do
    ...
    it 'is still filled with empty pieces after YAML dump/load' do
    ...
    describe '#set_piece' do
      context 'after putting Knight at (3, 4)' do
        ...
        it 'still has a Knight there after YAML dump/load' do
          expect(dump_and_load.get_piece(row, col).get_type).to eql(:knight)
        end  
